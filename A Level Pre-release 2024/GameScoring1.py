...
#When two or more puzzle patterns overlap in their 3x3 grid, award 10 extra points
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if self.__SymbolsLeft == 0:
                Finished = True
            multipleSymbolsBlocked = []
            for row in range(self.__GridSize, 0, -1):
                for col in range(1, self.__GridSize + 1):
                    CurrentCell = self.__GetCell(row, col)
                    count = 0
                    for letter in letterList:
                        if letter in CurrentCell.GetNotAllowed():
                            count += 1
                    if count > 1:
                        multipleSymbolsBlocked.append([row, col])
            print(lastMultipleSymbolsBlocked)
            print(multipleSymbolsBlocked)
            if len(multipleSymbolsBlocked) > len(lastMultipleSymbolsBlocked):
                self.__Score += 10
            elif len(multipleSymbolsBlocked) < len(lastMultipleSymbolsBlocked):
                self.__Score -= 10
            lastMultipleSymbolsBlocked = multipleSymbolsBlocked
        print()
        self.DisplayPuzzle()
        print()
        return self.__Score
