...
#When finishing game, any letters not part of a puzzle are subtracted from the final score.
                ...
                for row in range(self.__GridSize, 0, -1):
                    for column in range(1, self.__GridSize + 1):
                        ...
                        if self.CheckforMatchWithPattern(row, column) == 10:
                            CorrectCount += 1
                AmountToDeduct = SymbolCount - CorrectCount
                self.__Score -= AmountToDeduct
        print()
        self.DisplayPuzzle()
        print()
        return self.__Score
