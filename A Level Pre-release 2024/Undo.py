...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            lastSymbolsLeft = self.__SymbolsLeft    #START
            self.__SymbolsLeft -= 1
            CurrentCell = self.__GetCell(Row, Column)
            lastSymbol = CurrentCell.GetSymbol()
            lastNotAllowedSymbols = CurrentCell.GetNotAllowedSymbols()   
            lastScore = self.__Score
            if CurrentCell.CheckSymbolAllowed(Symbol):
                CurrentCell.ChangeSymbolInCell(Symbol)
                AmountToAddToScore, affectedCells = self.CheckforMatchWithPattern(Row, Column)
                if AmountToAddToScore > 0:
                    self.__Score += AmountToAddToScore
            self.DisplayPuzzle()
            undo = input("Would you like to undo the last move? ")
            valid = False
            while not valid:
                if undo == 'y':
                    self.__SymbolsLeft = lastSymbolsLeft
                    CurrentCell.ChangeSymbolInCell(lastSymbol)
                    CurrentCell = self.__GetCell(Row, Column)
                    self.__Score = lastScore
                    for coordinate in affectedCells:
                        CurrentCell = self.__GetCell(coordinate[0], coordinate[1])
                        CurrentCell.ResetNotAllowedSymbols()
                        lastNotAllowedSymbols = CurrentCell.GetLastNotAllowed()
                        for symbol in lastNotAllowedSymbols:
                            CurrentCell.AddToNotAllowedSymbols(symbol)
                    valid = True
                    pass
                elif undo == 'n':
                    print("You have chosen not to undo")
                    valid = True
                else:
                ...
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            affectedCells = [[StartRow, StartColumn], [StartRow, StartColumn + 1], [StartRow, StartColumn + 2], [StartRow - 1, StartColumn + 2], [StartRow - 2, StartColumn + 2], [StartRow - 2, StartColumn + 1], [StartRow - 2, StartColumn], [StartRow - 1, StartColumn], [StartRow - 1, StartColumn + 1]]
                            for coordinate in affectedCells:
                                CurrentCell = self.__GetCell(coordinate[0], coordinate[1])
                                for symbol in CurrentCell.GetNotAllowedSymbols():
                                    CurrentCell.AddToLastNotAllowed(symbol)
...
class Cell():
    def __init__(self):
        ...
        self.__LastSymbolsNotAllowed = []

    def GetLastNotAllowed(self):
        return self.__LastSymbolsNotAllowed

    def AddToLastNotAllowed(self, SymbolToAdd):
        self.__LastSymbolsNotAllowed.append(SymbolToAdd)
